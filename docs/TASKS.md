...
- 🔄 ✨ **Item 2 — God Skills** (roadmap #2 `approved` · spec [`specs/god-skills.md`](specs/god-skills.md)) —
  ...
  - [ ] **Pyroclasm** *(item 2 · spec [`specs/god-skills.md`](specs/god-skills.md) → "Pyroclasm" — **now fully spec'd**, Josh 2026-06-12)* — add `kind:'interval+autotarget'` entry + a nearest-cluster helper + updater branch (heavy spawn already removed). Build with `mpCost` from the start: **per-emit** (one charge per interval-fired lane, no per-second — like Trail), #8.9 two-table cost+damage model. Forms: **Eruption** (single big finale) / **Firewall** (advancing walls). Ascensions: **Wyrmspine** (🐉, re-erupting finale + heal) · **Riftmaw** (🔥, run-long erratic fissure — *intentionally* breaks Eruption's shape, footprint-EXEMPT) · **Dragonmarch** (🐉, dragonfire walls + heal carpet) · **Hellfront** (🔥, *advancing chaos walls* — reshaped to keep Firewall's shape, footprint-EXEMPT, movement-relative cost). All detail in the spec.
