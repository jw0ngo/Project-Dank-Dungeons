...
- ◻️ 🔧 **FX asset reorg — update the `.src` paths** (↳ from ART, 2026-06-11) — `assets/fx/` is now foldered **by owner** (`cilia/` for the fire-god kit, `_shared/` for god-agnostic FX) per `assets/fx/README.md`. The files moved (Artist, `git mv`); **only the hardcoded paths in `index.html` need updating.** ⚠️ **Coupling: the file move + these path edits must reach `main` in the SAME push** — a half-migration deploys 404'd FX (degrades to procedural). Greppable old→new (sprite-var anchor; line numbers drift):
