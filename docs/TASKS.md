...
- ◻️ 🎨 **God-Skill Action Bar — upgrade the shipped HUD chip row** (↳ from PM, Josh-approved 2026-06-12 · spec [`specs/mana-economy.md`](specs/mana-economy.md) Phase 3 "God-Skill Action Bar") — Phase 3 shipped a functional DOM chip row `#g-godskills` (key · emoji icon · mp/s · lit/dormant⚠/off✕). Upgrade it to a **WoW-style action bar**: (1) swap the emoji glyph for the **real shared skill icon** — same `CARD_ICON_ART`-keyed asset the draft card uses (Artist handoff below; emoji stays fallback until art lands); (2) **active/dormant tell = god-colored border** — **CORE:** static god-colored border when active, greyed when dormant (unambiguous state); **STRETCH:** the border lights up with **particles of the skill's god** (Cilia = fire-red embers), animated; (3) add a **per-god signature colour as a registry datum** (Cilia = fire-red) so the treatment is generic + future gods inherit it. **Scope = god-skill bar only** (unify with class-kit CD chips is a later vision — Josh: "god-skill bar now, unify later"). Deploy-affecting → push with Josh's auth.

- ◻️ 🎨 **Wire shared god-skill icons into draft card + action bar** (↳ from PM, 2026-06-12 · pairs with the Artist handoff + the action-bar task above) — once the Artist delivers the 3 Cilia skill icons, wire each as a `CARD_ICON_ART`-keyed asset used in **both** `_paintDraft` (the level-up card) **and** the `#g-godskills` slot — one asset, two sites (no duplicate icon set). Confirm the draft cards + god skills carry a stable `iconKey` to key off (not `card.name`). Folds together with the existing "Level-up screen art-direction pass" image-icon override.
