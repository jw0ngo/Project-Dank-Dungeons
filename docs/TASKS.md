...
- ◻️ 🔧 **Fold `assets/char/` by faction — run the migration atomically** (↳ from ART, 2026-06-11) — fold the 248 flat char sprites into `player/` · `goblins/` · `wolves/` and repoint the ~200 manifest paths. **One command does both:** `python tools/fold-assets.py --domain char --apply` (`git mv` + rewrites every `'assets/char/<f>'`→`'assets/char/<faction>/<f>'` in `index.html`, then self-verifies every path resolves). Artist dry-ran it: 248 routed (0 unmapped), 200 rewrites, 48 unwired files moved without a rewrite (the hurt poses + wolfmother — adopt foldered paths when wired). **Must be ONE commit** — files + manifest are inseparable (half-done 404s *all* char art → blocky procedural fallback, which `node --check` won't catch). After: `node --check` the extracted `<script>` + `python dev.py` → confirm sprites render. *(Scheme: `assets/README.md`. Phase 2 = same tool for `tile/`, later.)*
...
- ◻️ 🎨 **Chaosfire + dragonfire ground-circle sprites** (↳ from ART, 2026-06-11) — two painted filled-disc FX for the burning **ground-circle** substances (`assets/fx/cilia/chaosfire-circle.png` crimson · `assets/fx/cilia/dragonfire-circle.png` prismatic). Black-bg, 512², `'lighter'`-ready, floored to true black. **Pure visual swap — no hitbox/`f.r` change.**
