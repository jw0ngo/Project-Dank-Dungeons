...
- 🔄 ✨ **Item 2 — God Skills** (roadmap #2 `approved` · spec [`specs/god-skills.md`](specs/god-skills.md)) —
  ...
  - [x] **Pyroclasm — built** *(item 2 · spec [`specs/god-skills.md`](specs/god-skills.md) → "Pyroclasm")* — **done ENG 2026-06-12.** Generic pool entry `IMBUE_PATHS.cilia.pyroclasm` (`kind:'pyroclasm'`) + `gTickPyroclasm` + `gPyroTarget`/`gPyroPushPillar`/`gPyroFireLane`/`gPyroFireWalls` + a `case 'pyroclasm'` dispatch → acquire/rank-up/forks/toggle/HUD all free. Every `PYRO_INTERVAL` (~3s) it **auto-aims at the nearest enemy** (`gPyroTarget`, ≤`PYRO_RANGE`; holds fire + charges nothing when none) and fires a lane of `gFirePillars`. **All per-emit** (one `mpLaneByRank` charge per lane; effective mp/s = cost÷interval = BB-parity 2→45). Damage = independent `pillarDmgByRank` (45× vs cost 22.5×); Firewall `dmgMult 0.35` (many pillars) vs Eruption `1.0` (few + a `PYRO_FINALE_SCALE`/`DMG` finale) keeps the totals balanced (ST nuke ≈ AOE spread). **Forms:** Eruption (colossal finale) / Firewall (`PYRO_WALL_COUNT` advancing perpendicular walls). **Ascensions** reuse substance grounds via `gPyroPushPillar`: 🐉 Wyrmspine (finale re-erupts `PYRO_REERUPT`× + dragonfire heal) · 🔥 Riftmaw (`PYRO_RIFT_SCATTER` erratic chaos pillars past the aim) · 🐉 Dragonmarch (dragonfire walls + heal carpet) · 🔥 Hellfront (+2 chaos walls). Plumbed per-pillar `burnDur` + `scale` through `gFirePillars` (legacy heavy callers fall back); pillars now also hit the training dummy (DPS-testable). `node --check` + economy eval verified. **All three Cilia God Skills now built (Burning Body · Trail of Embers · Pyroclasm).**
    ...
    - **↻ Mouse-distance finale + range scaling (Josh 2026-06-12):** the Eruption **finale lands AT the mouse cursor** (lead-up pillars distribute from the player out to it), clamped to `maxRange = VW*0.5 * rangeFrac`. New per-rank `rangeFracByRank` (0.30→**1.00**) → at full evolution the finale reaches the **screen's left/right edge** (camera centers the player, so `VW/2` = edge). `gPyroAimDist` computes the clamp (used by both fire + telegraph). Lane (base+Eruption) distributes `count` pillars start→aimDist; Firewall keeps fixed advancing-wall distances.
    - **↻ Riftmaw reworked → a cursor-placed map-crossing burning rift (Josh 2026-06-12, iterated):** placed at
      the **mouse cursor (anywhere on screen)**, perpendicular to your aim, extending **both ways** to
      `±min(VW, PYRO_RIFT_HALFSPAN)` ("across the map", never on you by default), erupting **outward from the
      cursor** (`PYRO_RIFT_DELAY`) so it tears open. **The aim indicator is a clean STRAIGHT line of circles**
      (`PYRO_RIFT_TELE_STEP` 64, no jitter) while **the actual rift is a CONDENSED, JAGGED single line of
      chaosfire pillars** (`PYRO_RIFT_STEP` 32 + deterministic `PYRO_RIFT_JITTER`) — `gPyroPlan(..., forTelegraph)`
      branches the two. The chaos fire **lingers + burns for 20s** (`PYRO_RIFT_GROUND_LIFE`/`PYRO_RIFT_BURN`
      1200f; `gPyroPushPillar` gained a `groundLife` arg + passes `burnFrames`). Per-pillar dmg ×`PYRO_RIFT_DMG`
      (0.4). **⚠ Perf watch:** ~45 pillars/cast, each a 20s ground patch → up to ~135 persistent patches with the
      7s recast; fine on a decent machine but flag if it dips (lighten via bigger `PYRO_RIFT_STEP` / shorter
      `PYRO_RIFT_GROUND_LIFE`). **Open call (flag for Josh):** the rift sits **perpendicular** to your aim (clean
      "place a crossing wall"); if you want it **along** your aim instead, it's a one-line flip. **Test knobs:**
      the `PYRO_RIFT_*` set + `rangeFracByRank`.
  ...
  - [ ] ✨ **Rank-10 Epic-ness rework — bring the clear misses up to the Riftmaw benchmark** (↳ from PM, Josh 2026-06-12 · spec [`specs/god-skills.md`](specs/god-skills.md) → "The Rank-10 Epic-ness Bar"). Riftmaw (map-spanning chaos rift, tears open, 20s linger) is the bar; two Pyroclasm capstones are **below it** and get raised NOW (the rest later — see the tracking task):
    - **Hellfront** (🔥 Firewall) — today just **+2 advancing walls** (`gPyroPlan` firewall branch `index.html:~4382`; `walls = PYRO_WALL_COUNT + 2`). Re-spec to a **map-sweeping TIDE of chaosfire walls**: wall after wall marching from the aim line out to the screen edges over ~1–2s, **leaving a lingering chaosfire field in its wake** (reuse the chaosfire-ground spawn already in `gPyroPushPillar`). Keeps Firewall's wall shape; cost movement-relative; footprint-EXEMPT. Knobs: extend `PYRO_WALL_COUNT`/`GAP`/`STAGGER` to span ~`VW`, add a wake-field life. **Pair it:** raise **Dragonmarch** (🐉) to the matching dragonfire wall-tide + heal-carpet at the same time (cheap once the sweep exists — keeps the Firewall pair coherent).
    - **Wyrmspine** (🐉 Eruption) — today the finale **re-erupts 3× in place** (`gPyroFire` wyrmspine branch `~4419`; `PYRO_REERUPT`). Re-spec to the **dragon-spine MIRROR of Riftmaw**: a colossal line of giant **dragonfire** pillars erupting in sequence **across the map** from the cursor (both directions), each laying a **dragonfire heal-carpet** → a vast map-spanning healing spine. Mirror Riftmaw's `gPyroPlan` rift branch (`~4358`) but **ordered/majestic** (no jagged jitter) + dragonfire substance + heal-ground (the `gPyroPushPillar` dragonfire path). Both Eruption capstones now map-scale (epic bar > shape-coherence at the capstone — see spec).
    - **Verify:** `node --check` + in-browser — Hellfront sweeps the whole arena & leaves a burning wake; Wyrmspine's spine spans the map & heals along its length; both read as a capstone, not a bigger rank-9.
  - [ ] 🔧 **Raise the remaining below-bar rank-10 leaves to the Epic-ness Bar during the god-skill feel-pass** (↳ from PM, Josh 2026-06-12 — "fix clear misses first, raise the rest later") — Trail (Chaoswake / Chaos Steps / Wyrmwake / Dragonfeet) + Burning Body (Eye of Chaos / Dragonbreath / Dragonheart) + Pyro **Dragonmarch** (if not done with Hellfront) are battlefield-*local*, below the map-scale bar. Raise each toward the 5 ingredients (map-scale footprint · dramatic reveal · lingering reshape · player-placed · tonal) in the dedicated feel-tuning pass before the god-skills tag. Not a blocker for the Hellfront/Wyrmspine fix above.
