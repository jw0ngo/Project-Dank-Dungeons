...
**Owner: the Engineer / CTO.** Only the owner flips a status here; **any agent may file a task in** (tag it
`(↳ from <role>, date)`). Conventions + the status/type legend live in the hub: [`../TASKS.md`](../TASKS.md).
Strategy/priority: [`../ROADMAP.md`](../ROADMAP.md). Sibling docs: [`pm.md`](pm.md) · [`artist.md`](artist.md).
...
- ◻️ 🎨 **Skin the Obelisk POI with its sprite** (↳ from ART, 2026-06-12) — the §12b Obelisk system (`gObelisks`, `_drawObeliskStone` `:16096`) draws a tiny ~42px procedural stone marker; replace it with the new art (same pattern as chest/barrel — skin the existing entity, keep the procedural fallback). Cutout committed: **`assets/world/obelisk.png`** (247×512, ~210 KB — transparent, feet-anchored, base at the cutout bottom; dark rune-stone with purple glowing runes + base crystals). Manifest (auto-loads via `gInitArt`):
  ...
  ```
  - **Draw:** in `_drawObeliskStone` (`:16096`), if `gArtReg['world.obelisk']` is loaded → draw it **feet-anchored** at the obelisk's `(sx, sy)` (bottom of the sprite at ~`sy+14`, where the procedural base shadow sits), AR 247/512≈0.48; **suggested height ~80–96 px** (tune in-game — it's a landmark, bigger than the old 42px marker). Keep the procedural stone as fallback (`haveSpr` pattern). All the surrounding overlays stay as-is (the `#44ffcc` glow ring, channel-fill ring, rune text, "STAND TO CHANNEL" prompt — they draw around the marker).
  - **`used` state:** today it dims to dark inert stone (`globalAlpha 0.4`); for the sprite, draw at reduced alpha (~0.4) when `ob.used` (a cheap dim; a true desaturate needs a filter — your call).
  - **No size coupling:** draw size is **independent of `OBELISK_RADIUS`** (the 96px channel ring is proximity-based, not a hitbox) — just keep the sprite comfortably *inside* the ring so the ring still reads as the stand-zone. Render-only, no sim/MP change. **HiDPI:** standard sprite blit; the 512-tall source has plenty of detail headroom at ~96px.
  - **Fold note:** delivered FLAT (`assets/world/obelisk.png`) to match the current un-folded `assets/world/`. When the pending `world` fold runs, obelisk → `world/_shared/` (cross-area POI) — **add `obelisk` to `fold-assets.py FAMILIES['world']`** so the fold sweeps it. Source master `art/world/obelisk.png`. Deploy-affecting → push with Josh's auth.
