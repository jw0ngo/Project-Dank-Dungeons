...
the agent roster, and the recursive-learning habit. Creative direction (vision, story, feel) is set by
the **Creative Director** (Josh) in **`studio/CREATIVE_MANIFESTO.md`** and sits above all other roles.
At the end of a substantive session, crystallize your highest-level lessons into your role's
`LEARNINGS.md` (engineer: `docs/learnings/engineer.md`).
...
- **Product Manager — opt-in.** Owns *what* and *why*: the roadmap. Operating context is
  `product/AGENTS.md` (model: `docs/PRODUCT_MANIFESTO.md`). Switch into it with **`/pm`**.
